...
void IROptimizer::copyPropagation(BasicBlock& block, bool& changed) {
    ...
    for (auto& inst : block.instructions) {
        ...
        if (inst.opcode == IROpcode::ASSIGN) {
            if (inst.arg1->type == OperandType::VARIABLE || 
                inst.arg1->type == OperandType::CONSTANT ||
                inst.arg1->type == OperandType::TEMP) {
                // 查找传递后的源操作数（处理间接引用，如a=b, b=5，则a直接映射到5）
                std::shared_ptr<Operand> source = inst.arg1;
                // 递归查找最终源（如果是变量/临时变量且存在映射）
                while (source->type == OperandType::VARIABLE || source->type == OperandType::TEMP) {
        ...
        auto replaceOperand = [&](std::shared_ptr<Operand>& arg) {
            if (arg && (arg->type == OperandType::VARIABLE || arg->type == OperandType::TEMP)) {
...
void IROptimizer::deadCodeElimination(std::vector<BasicBlock>& blocks, const LiveAnalyzer& liveAnalyzer,bool& changed){
    ...
    for (auto& block : blocks) {
        ...
        for (auto& inst : block.instructions) {
            ...
            if (inst.result && 
                (inst.result->type == OperandType::VARIABLE || inst.result->type == OperandType::TEMP)) {
...
void IROptimizer::algebraicSimplification(BasicBlock& block, bool& changed) {
    for (auto& inst : block.instructions) {
        switch (inst.opcode) {
            case IROpcode::ADD:
                // x + 0 = x
                if (isConstant(inst.arg2) && inst.arg2->getConstantValue() == 0) {
                    // std::cout<<"old: " + inst.toString()<<std::endl;
                ...
                // 0 + x = x
                else if (isConstant(inst.arg1) && inst.arg1->getConstantValue() == 0) {
                    // std::cout<<"old: " + inst.toString()<<std::endl;
            ...
            case IROpcode::SUB:
                ...
                // x - x = 0
                else if (inst.arg1->id == inst.arg2->id) {
                    // std::cout<<"old: " + inst.toString()<<std::endl;
            ...
            case IROpcode::MUL:
                // x * 0 = 0
                if ((isConstant(inst.arg1) && inst.arg1->getConstantValue() == 0) ||
                    (isConstant(inst.arg2) && inst.arg2->getConstantValue() == 0)) {
                    // std::cout<<"old: " + inst.toString()<<std::endl;
                    inst = IRInstruction(IROpcode::ASSIGN, inst.result, createConstant(0));
                    // std::cout<<"new: " + inst.toString()<<std::endl;
                    changed = true;
                ...
                // x * 1 = x
                else if (isConstant(inst.arg2) && inst.arg2->getConstantValue() == 1) {
                    // std::cout<<"old: " + inst.toString()<<std::endl;
                    inst = IRInstruction(IROpcode::ASSIGN, inst.result, inst.arg1);
                    // std::cout<<"new: " + inst.toString()<<std::endl;
                    changed = true;
                ...
                // 1 * x = x
                else if (isConstant(inst.arg1) && inst.arg1->getConstantValue() == 1) {
                    // std::cout<<"old: " + inst.toString()<<std::endl;
                    inst = IRInstruction(IROpcode::ASSIGN, inst.result, inst.arg2);
                    // std::cout<<"new: " + inst.toString()<<std::endl;
                    changed = true;
    ...
    for (size_t i = 0; i < block.instructions.size(); i++) {
        ...
        // 当变量被重新定义时，清除相关缓存
            ...
            for (auto it = exprCache.begin(); it != exprCache.end();) {
                ...
                if (it->second.first->id == inst.result->id || 
                    it->first.find(inst.result->id) != std::string::npos) {
...
bool IROptimizer::isConstant(const std::shared_ptr<Operand>& op) {
    return op->isEffectivelyConstant();
}
...
void IROptimizer::updateVariableCounts(FunctionInfo& func) {
    ...
    auto processOperand = [&](const std::shared_ptr<Operand>& op) {
        ...
        if (op->type == OperandType::VARIABLE || op->type == OperandType::PARAM) {
            // 普通变量：直接用字符串ID去重
            variableIds.insert(op->id);
        } else if (op->type == OperandType::TEMP) {
            // 临时变量：解析ID为整数（假设格式为"0", "1", ...）
            try {
                int tempId = std::stoi(op->id);
                tempIds.insert(tempId);  // 用整数ID去重
            } catch (const std::invalid_argument& e) {
                std::cerr << "警告：无效的临时变量ID格式: " << op->id << std::endl;
            } catch (const std::out_of_range& e) {
                std::cerr << "警告：临时变量ID超出范围: " << op->id << std::endl;
            }
        }
    };
